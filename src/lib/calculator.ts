...
export function calculateSodium(
  ...
  if (duration < 60) {
    ...
    recommendation = `Light sodium needs: ${Math.round(adjustedPerHour)}mg/hour (~${Math.round(perHourGrams * 100) / 10}g salt). Electrolyte drink sufficient.`;
  } else if (adjustedPerHour < 600) {
    recommendation = `Moderate sodium: ${Math.round(adjustedPerHour)}mg/hour (~${Math.round(perHourGrams * 100) / 10}g salt). Use electrolyte tablets or add salt to food.`;
  } else {
    recommendation = `High sodium: ${Math.round(adjustedPerHour)}mg/hour (~${Math.round(perHourGrams * 100) / 10}g salt). Consider multiple electrolyte sources.`;
  }
